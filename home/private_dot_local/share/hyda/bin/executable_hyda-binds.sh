#!/usr/bin/env bash
# description: display hyprland keybinds via rofi

get_binds() {
  if ! command -v hyprctl &>/dev/null; then
    log_error "hyprctl not found — is Hyprland running?"
    exit 1
  fi
...
main() {
  if ! command -v rofi &>/dev/null; then
    log_error "rofi not found — required for displaying keybinds"
    exit 1
  fi

  log_info "Fetching Hyprland binds..."
  local binds
  binds="get_binds | sort -u | parse_bindings"

  if [[ -z "$binds" ]]; then
    log_error "No binds detected — ensure Hyprland is running and accessible"
    exit 1
  fi

  $binds | rofi -dmenu -i -p 'Keybinds' \
  -theme ~/.config/rofi/configs/keybinds.rasi \
  -kb-accept-entry "" -kb-accept-custom "" -me-accept-entry ""
}

if [[ $# -gt 0 ]]; then
  log_error "hyda keybinds doesn't take arguments — just run it as is"
  exit 1
else
  main
fi
